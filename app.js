...
const PROFILE_KEY   = 'cinereads_profile_v1';

const MOVIE_PLATFORMS = [
  { id: 'netflix',    label: 'Netflix',        color: '#e50914' },
  { id: 'hulu',       label: 'Hulu',           color: '#1ce783', textDark: true },
  { id: 'max',        label: 'Max',            color: '#002be7' },
  { id: 'disney',     label: 'Disney+',        color: '#113ccf' },
  { id: 'prime',      label: 'Prime Video',    color: '#00a8e1' },
  { id: 'appletv',    label: 'Apple TV+',      color: '#444' },
  { id: 'peacock',    label: 'Peacock',        color: '#0f5799' },
  { id: 'paramount',  label: 'Paramount+',     color: '#0064ff' },
  { id: 'tubi',       label: 'Tubi',           color: '#fa500f' },
  { id: 'theaters',   label: 'In Theaters',    color: '#7b2d8b' },
  { id: 'dvd',        label: 'DVD / Blu-ray',  color: '#555' },
  { id: 'youtube',    label: 'YouTube',        color: '#ff0000' },
];

const BOOK_PLATFORMS = [
  { id: 'amazon',     label: 'Amazon',         color: '#ff9900', textDark: true },
  { id: 'kindle',     label: 'Kindle',         color: '#00a8e1' },
  { id: 'library',    label: 'Library',        color: '#2e7d32' },
  { id: 'bn',         label: 'Barnes & Noble', color: '#1f5c99' },
  { id: 'audible',    label: 'Audible',        color: '#f5a623', textDark: true },
  { id: 'applebooks', label: 'Apple Books',    color: '#e8334a' },
  { id: 'google',     label: 'Google Play',    color: '#4285f4' },
  { id: 'libby',      label: 'Libby',          color: '#e85a1b' },
  { id: 'gutenberg',  label: 'Project Gutenberg', color: '#3a6e41' },
  { id: 'bookshop',   label: 'Bookshop.org',   color: '#2c6e49' },
];
...
/* ---- App state ---- */
let state = {
  currentPage:    'browse',   // 'browse' | 'mylist' | 'profile'
  currentTab:     'movies',
  search:         '',
  genreFilter:    'all',
  statusFilter:   'all',
...
function saveProfileData() {
  try {
    ...
    console.warn('Reel&Reed: localStorage write failed (profile).');
  }
}
...
function updateProfileBtn() {
  ...
  if (!btn) return;
  btn.textContent = profileData.avatar || '👤';
  btn.classList.toggle('active', state.currentPage === 'profile');
}

function getItem(id) {
  return userData[id] || { favorite: false, completed: false, rating: 0, review: '', platforms: [], customPlatform: '' };
}
...
function buildCardPlatforms(item) {
  const ud       = getItem(item.id);
  const list     = getPlatformList(item.type);
  const selected = (ud.platforms || []).map(id => list.find(p => p.id === id)).filter(Boolean);
  const custom   = ud.customPlatform || '';

  if (!selected.length && !custom) return '';

  const MAX_SHOW = 2;
  const shown    = selected.slice(0, MAX_SHOW);
  const extra    = selected.length - MAX_SHOW;

  const chips = shown.map(p =>
    `<span class="card-plat-chip colored" style="--plat-color:${p.color}">${escHtml(p.label)}</span>`
  ).join('');

  const customChip = custom && !selected.length
    ? `<span class="card-plat-chip">${escHtml(custom.length > 14 ? custom.slice(0, 13) + '…' : custom)}</span>`
    : '';

  const moreTag = extra > 0
    ? `<span class="card-plat-more">+${extra}</span>`
    : '';

  return `<div class="card-platforms">${chips}${customChip}${moreTag}</div>`;
}
...
function showPage(page) {
  ...
  document.getElementById('filtersSection').style.display = isBrowse  ? '' : 'none';
  document.getElementById('statsSection').style.display   = isBrowse  ? '' : 'none';
  document.getElementById('browseMain').style.display     = isBrowse  ? '' : 'none';
  document.getElementById('myListPage').style.display     = isMyList  ? '' : 'none';
  document.getElementById('profilePage').style.display    = isProfile ? '' : 'none';
...
/* ======================================================
   WHERE TO FIND — PLATFORM SECTION
====================================================== */
function getPlatformList(type) {
  return type === 'movie' ? MOVIE_PLATFORMS : BOOK_PLATFORMS;
}

function renderPlatformSection(item) {
  const ud        = getItem(item.id);
  const list      = getPlatformList(item.type);
  const selected  = ud.platforms || [];
  const custom    = ud.customPlatform || '';
  const heading   = item.type === 'movie' ? 'Where to Watch' : 'Where to Read';

  const chipsHtml = list.map(p => {
    const isActive = selected.includes(p.id);
    const style    = isActive ? `style="--plat-color:${p.color}"` : '';
    return `<button class="platform-chip${isActive ? ' active' : ''}" data-pid="${p.id}" ${style}>${escHtml(p.label)}</button>`;
  }).join('');

  const customActiveHtml = custom
    ? `<div class="platform-custom-active">
         &#128205; ${escHtml(custom)}
         <button class="platform-custom-clear" id="btnClearCustomPlatform" title="Remove">&#10005;</button>
       </div>`
    : '';

  document.getElementById('modalPlatforms').innerHTML = `
    <h3>${heading}</h3>
    <div class="platform-grid" id="platformGrid">${chipsHtml}</div>
    <div class="platform-custom-row">
      <input type="text" id="customPlatformInput" placeholder="Custom platform&hellip;" value="${escHtml(custom)}" maxlength="80">
      <button id="btnSaveCustomPlatform">Add</button>
    </div>
    ${customActiveHtml}
  `;

  // Chip toggles
  document.getElementById('platformGrid').querySelectorAll('.platform-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      const pid = btn.dataset.pid;
      togglePlatform(item.id, pid, btn);
    });
  });

  // Save custom
  document.getElementById('btnSaveCustomPlatform').addEventListener('click', () => {
    const val = document.getElementById('customPlatformInput').value.trim();
    saveCustomPlatform(item.id, val);
  });

  document.getElementById('customPlatformInput').addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      e.preventDefault();
  ...
  // Clear custom
  const clearBtn = document.getElementById('btnClearCustomPlatform');
  if (clearBtn) {
    clearBtn.addEventListener('click', () => saveCustomPlatform(item.id, ''));
  }
}

function togglePlatform(id, platformId, btnEl) {
  const ud        = getItem(id);
  ...
  setItem(id, { platforms });

  // Update button state in-place (avoid full re-render)
  const isNowActive = idx === -1;
  const platObj     = [...MOVIE_PLATFORMS, ...BOOK_PLATFORMS].find(p => p.id === platformId);
  btnEl.classList.toggle('active', isNowActive);
  if (isNowActive && platObj) {
    btnEl.style.setProperty('--plat-color', platObj.color);
  } else {
    btnEl.style.removeProperty('--plat-color');
  }

  renderGrid();
}

function saveCustomPlatform(id, text) {
  setItem(id, { customPlatform: text });
  showToast(text ? `Platform saved: ${text}` : 'Custom platform removed');
  // Re-render just the section to update the active tag
  const item = getAllItems().find(c => c.id === id);
  if (item) renderPlatformSection(item);
  renderGrid();
}
...
function renderProfilePage() {
  ...
  if (state.editingProfile) {
    const currentAvatar = state.pendingAvatar || p.avatar || '🎬';
      ...
      <div class="profile-inner">
        <div class="profile-edit-card">
          ...
          <div class="avatar-edit-row">
            <div class="avatar-circle clickable" id="editAvatarCircle">${currentAvatar}</div>
            <div class="avatar-edit-info">
              <span>Your avatar</span>
              <button class="btn-change-avatar" id="btnTogglePicker">Change Avatar</button>
            </div>
          </div>

          <div class="avatar-picker-grid" id="avatarPickerGrid">
            ${AVATARS.map(a =>
              `<button class="avatar-opt${a === currentAvatar ? ' selected' : ''}" data-avatar="${a}" aria-label="${a}">${a}</button>`
            ).join('')}
          </div>

          <div class="profile-edit-form">
            <div class="form-group">
              <label for="editName">Display Name <span class="required">*</span></label>
              <input type="text" id="editName" value="${escHtml(p.name || '')}" placeholder="Your name" maxlength="40" autocomplete="off">
            </div>
            <div class="form-group">
              <label for="editBio">Bio</label>
              <textarea id="editBio" rows="3" placeholder="Tell us about yourself&hellip;" maxlength="200">${escHtml(p.bio || '')}</textarea>
            </div>
            <div class="form-group">
              <label for="editGenre">Favorite Genre</label>
              <select id="editGenre">
                <option value="">No preference</option>
                ${Object.keys(GENRE_CONFIG).sort().map(g =>
                  `<option value="${g}"${g === p.favoriteGenre ? ' selected' : ''}>${g}</option>`
                ).join('')}
              </select>
            </div>
          </div>
          ...
          <div class="profile-edit-actions">
            ...
            <button class="btn-cancel-edit" id="btnCancelProfile">Cancel</button>
          </div>
        </div>
      </div>
    `;
    ...
    document.getElementById('btnTogglePicker').addEventListener('click', () => {
      document.getElementById('avatarPickerGrid').classList.toggle('open');
    });
    ...
    page.querySelectorAll('.avatar-opt').forEach(btn => {
      btn.addEventListener('click', () => {
        page.querySelectorAll('.avatar-opt').forEach(b => b.classList.remove('selected'));
        btn.classList.add('selected');
        state.pendingAvatar = btn.dataset.avatar;
        document.getElementById('editAvatarCircle').textContent = btn.dataset.avatar;
      });
    });
    ...
    document.getElementById('btnSaveProfile').addEventListener('click', () => {
      ...
      profileData = {
        ...
        favoriteGenre: document.getElementById('editGenre').value,
        avatar:        state.pendingAvatar || p.avatar || '🎬',
        joinedAt:      profileData.joinedAt || Date.now(),
      };
      saveProfileData();
    ...
    const all       = getAllItems();
    const ratedAll  = all.filter(c => getItem(c.id).rating > 0);
    const avgRating = ratedAll.length
      ? (ratedAll.reduce((s, c) => s + getItem(c.id).rating, 0) / ratedAll.length).toFixed(1)
      : null;
    const completed = all.filter(c => getItem(c.id).completed).length;
    const favorites = all.filter(c => getItem(c.id).favorite).length;
    const joinDate  = p.joinedAt
      ? new Date(p.joinedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
      : null;
    const favCfg = p.favoriteGenre && GENRE_CONFIG[p.favoriteGenre];

    // First-time experience — prompt to set up
    const isNew = !p.name;
    ...
    page.innerHTML = `
      <div class="profile-inner">
        <div class="profile-view-card">
          ...
          <h2 class="profile-name">${escHtml(p.name || 'New Viewer')}</h2>
          ${p.bio
            ? `<p class="profile-bio">${escHtml(p.bio)}</p>`
            : `<p class="profile-bio muted">${isNew ? 'Set up your profile to personalize your experience.' : 'No bio yet.'}</p>`
          }
          ${p.favoriteGenre && favCfg
            ? `<div class="profile-genre-chip">${favCfg.emoji} ${p.favoriteGenre}</div>`
            : ''
          }
          ${joinDate ? `<p class="profile-since">Member since ${joinDate}</p>` : ''}
        ...
        <div class="profile-stats-card">
          <div class="profile-section-label">Your Activity</div>
          <div class="pstats-grid">
            <div class="pstat">
              <div class="pstat-num">${all.length}</div>
              <div class="pstat-label">Titles</div>
            </div>
            <div class="pstat">
            ...
            <div class="pstat">
              <div class="pstat-num">${favorites}</div>
              <div class="pstat-label">Favorites</div>
            </div>
            <div class="pstat">
              <div class="pstat-num">${avgRating !== null ? avgRating : '&mdash;'}</div>
...
function init() {
  ...
  document.getElementById('profileBtn').addEventListener('click', () => {
    if (state.currentPage === 'profile') {
      // Return to the last browse tab
      const activeTab = document.querySelector('.tab-btn.active');
      state.editingProfile = false;
      state.pendingAvatar  = null;
      if (activeTab && activeTab.dataset.tab === 'mylist') {
        showPage('mylist');
      } else {
        showPage('browse');
      }
